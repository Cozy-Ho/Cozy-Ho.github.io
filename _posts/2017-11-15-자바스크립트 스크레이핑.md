...
`DHTML`은 클라이언트 쪽 스크립트가 페이지의 HTML 요소가 바뀜에 따라 바뀌는 HTML이나 CSS이다. 커서를 움직여야 버튼이 나타나거나, 클릭하면 배경색이 바뀌거나, Ajax 요청으로 새로운 컨텐츠가 나타날 수도 있다.
...
## 셀레니움
<br>
전 포스트에서 `requests`를 이용해 네이버에 로그인 하려했지만 실패했었다. 프론트 단에서 자바스크립트 처리를통해 로그인처리를 하기 때문인데, 셀레니움을 사용하면 쉽게 가능하다.
<br>
...
또 로그인 버튼을 눌러야하는데, <a href="https://cozy-ho.github.io/web%20scraping/2017/11/14/%ED%8F%BC%EA%B3%BC-%EB%A1%9C%EA%B7%B8%EC%9D%B8-%EB%9A%AB%EA%B8%B0.html" target="_blank">폼과 로그인 뚫기</a>포스트 에서 설명했듯이 폼 데이터를 처리하는 페이지에 요청을 보내야하기때문에 다른 URL이 필요하다.
<br>
...
- 속성 선택<br>- //@href는 href 속성이 있는 모든 노드를 선택한다.<br>- //a[@href='http://google.com']는 문서에서 구글을 가리키는 모든 링크를 선택한다.
...
## Congratulation
<br>
여러분은 이제 웹 페이지를 마음껏 돌아다니며 원하는 데이터를 모조리 긁어올 수 있다!

나중에 중요하게 포스팅 하겠지만 스크립트를 `절대로` 아무 사이트나 돌아니면서 실행하면 안된다 `절대로.` 잘 보호된 일부 사이트는 폼을 너무 빨리 전송하거나 너무 빨리 행동한다면 막힐 수 있다. 일반적인 사용자보다 압도적으로 많은 정보를 가져가거나 요구해도 웹 마스터의 주의를 끌고 차단 당할 수 있다.

따라서 멀티스레드로 속도를 끌어올리는 것 보다는 `time.sleep(3)` 으로 몇 초 정도 간격을 두는게 좋다.

다른건 몰라도 이것만은 꼭 지키자. 서버 자원을 과도하게 사용하면 법적으로 불리해질 뿐만 아니라, 소규모 사이트같은 경우에는 서버가 다운되게 할 수도 있기때문이다. 웹 사이트를 다운시키는 것은 윤리적으로 모호한 상황이 아니다. 그건 완전히 `잘못된 일`이다. 주의하자.
